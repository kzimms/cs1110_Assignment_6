...
class ClusterGroup(object):
    ...
    def __init__(self, ds, k, seed_inds=None):
        ...
        if seed_inds is not None:
            for i in seed_inds:
                #print `i`
                centroids.append(self._dataset.getContents()[i])
                
        else:
            data = self._dataset.getContents()
            index = []
            for num in range(k):
        ...
        for x in centroids:
            clusters.append(Cluster(self._dataset,x))
    ...
    def _nearest_cluster(self, point):
        ...
        for k in range(num-1):
            a = Cluster.distance(self.getClusters()[k], point)
            b = Cluster.distance(self.getClusters()[k+1], point)
            if a <= b and (a < minimum or minimum == 0):
                minimum = a
                mincluster = self.getClusters()[k]
            if b < a and (b < minimum or minimum == 0):
                minimum = b
                mincluster = self.getClusters()[k+1]
        #print `minimum`
        #print `mincluster`
    ...
    def _partition(self):
        ...
        #a = self.getClusters.getIndices()
        #a = self._dataset.getContents()
        for k in range(len(self.getClusters())):
            clust = self.getClusters()[k]
            clust.clear()
            #print `self._clusters`
            for point in self._dataset.getContents():
                #print `point`
                b = self._nearest_cluster(point)
                #print `b`
                b.addIndex(self._dataset.getContents().index(point))
    ...
    def _update(self):
        ...
        """
        changed = []
        for centroid in self.getClusters():
            a = centroid.updateCentroid()
            changed.append(a)
        if False in changed:
            return False
        return True
